...
REM 6.) using 7-zip or other utility, generate a CRC32 value for the file saved to c:\video folder and update the line "set video_file_crc32_value=E6242C96" with the correct CRC value
REM 7.) attach the drive to be tested to the machine this script will run on
REM 8.) under My Computer right click on the drive to be tested and click properties. using the number of bytes of the drive (for example 17,999,805,546,496 bytes for a 18TB drive)
REM		divide the number of bytes in the drive by the size of "movie_size" and enter the whole number into "num_copies" so we can fill the drive as much as possible 
REM 9.) launch the script as administrator by right clicking the file and "run as administrator", and follow the instructions. 
...
set results_file="C:\Users\%USERNAME%\Desktop\disk1_CRC_%start_date%.md5"
set video_file_location="C:\video\movie.mkv"
set video_file_crc32_value=E6242C96
set log_file="C:\Users\%USERNAME%\Desktop\disk1_log_%start_date%.txt"


REM SCRIPT START - DO NOT EDIT ANYTHING PAST THIS LINE
set tmp_file="C:\Users\%USERNAME%\Desktop\tmpp.txt"
...
REM FIND exit codes
REM │
REM │The following list shows each exit code and a brief description of its
REM │meaning:
REM │
REM │0
REM │    The search was completed successfully and at least one match was found.
REM │
REM │1
REM │    The search was completed successfully, but no matches were found.
REM │
REM │2
REM │    The search was not completed successfully. In this case, an error
REM │    occurred during the search, and FIND cannot report whether any matches
REM │    were found.
...
echo. >																																													%tmp_file% && type %tmp_file% && type %tmp_file% >> %log_file%
echo --------------------------------------------------------------------------------------------------------------- >																	%tmp_file% && type %tmp_file% && type %tmp_file% >> %log_file%
echo %date% %time% (Step 1 of 6) Performing quick SMART test on Drive "%drive_letter%:" >																												%tmp_file% && type %tmp_file% && type %tmp_file% >> %log_file%
echo --------------------------------------------------------------------------------------------------------------- >																	%tmp_file% && type %tmp_file% && type %tmp_file% >> %log_file%
echo. >																																													%tmp_file% && type %tmp_file% && type %tmp_file% >> %log_file%
title (Step 1 of 6) Performing quick SMART test on Drive "%drive_letter%:"
smartctl -t short %drive_letter%: >																																						%tmp_file% && type %tmp_file% && type %tmp_file% >> %log_file%
echo. >																																													%tmp_file% && type %tmp_file% && type %tmp_file% >> %log_file%
echo waiting 30 seconds to poll status again >																																			%tmp_file% && type %tmp_file% && type %tmp_file% >> %log_file%
...
for /f "tokens=*" %%O in ('smartctl -x %drive_letter%:\ ^| find /i "% of test"') do (
		...
		if errorlevel 0 (
		  echo %date% %time% Short SMART test on Disk "%drive_letter%:" -- %%O ... waiting 30 seconds to poll status again >															%tmp_file% && type %tmp_file% && type %tmp_file% >> %log_file%
		  goto :short_smart_in_progress
		)
	)
	
echo %date% %time% Short SMART test complete, proceeding with rest of script >																											%tmp_file% && type %tmp_file% && type %tmp_file% >> %log_file%
echo. >																																													%tmp_file% && type %tmp_file% && type %tmp_file% >> %log_file%
echo. >																																													%tmp_file% && type %tmp_file% && type %tmp_file% >> %log_file%
echo --------------------------------------------------------------------------------------------------------------- >																	%tmp_file% && type %tmp_file% && type %tmp_file% >> %log_file%
echo %date% %time% (Step 2 of 6) Copying %num_copies% copies of file "%video_file_location%" to disk "%drive_letter%:" or until the drive is full whichever occurs first >				%tmp_file% && type %tmp_file% && type %tmp_file% >> %log_file%
echo --------------------------------------------------------------------------------------------------------------- >																	%tmp_file% && type %tmp_file% && type %tmp_file% >> %log_file%
echo. >																																													%tmp_file% && type %tmp_file% && type %tmp_file% >> %log_file%
title (Step 2 of 6) Copying files to disk %drive_letter%

for /l %%A in (1,1,%num_copies%) do (
...
echo. >																																													%tmp_file% && type %tmp_file% && type %tmp_file% >> %log_file%
echo --------------------------------------------------------------------------------------------------------------- >																	%tmp_file% && type %tmp_file% && type %tmp_file% >> %log_file%
echo %date% %time% (Step 3 of 6) Performing CRC Check of files copied to Drive "%drive_letter%:" >																						%tmp_file% && type %tmp_file% && type %tmp_file% >> %log_file%
echo --------------------------------------------------------------------------------------------------------------- >																	%tmp_file% && type %tmp_file% && type %tmp_file% >> %log_file%
echo. >																																													%tmp_file% && type %tmp_file% && type %tmp_file% >> %log_file%
title (Step 3 of 6) Performing CRC Check of files copied to Drive "%drive_letter%:"
...
echo. >																																													%tmp_file% && type %tmp_file% && type %tmp_file% >> %log_file%
echo --------------------------------------------------------------------------------------------------------------- >																	%tmp_file% && type %tmp_file% && type %tmp_file% >> %log_file%
echo %date% %time% (Step 4 of 6) Performing full Check Disk of %drive_letter% using commnd chkdsk %drive_letter%: /f /r /x >																			%tmp_file% && type %tmp_file% && type %tmp_file% >> %log_file%
...
echo --------------------------------------------------------------------------------------------------------------- >																	%tmp_file% && type %tmp_file% && type %tmp_file% >> %log_file%
echo. > 																																												%tmp_file% && type %tmp_file% && type %tmp_file% >> %log_file%
title (Step 4 of 6) Performing full Check Disk of Drive "%drive_letter%:"
echo. > 																																												%tmp_file% && type %tmp_file% && type %tmp_file% >> %log_file%
...
echo. >																																													%tmp_file% && type %tmp_file% && type %tmp_file% >> %log_file%
echo --------------------------------------------------------------------------------------------------------------- > 																	%tmp_file% && type %tmp_file% && type %tmp_file% >> %log_file%
echo %date% %time% (Step 5 of 6) Performing quick format of drive "%drive_letter%:" using command "FORMAT %drive_letter%: /FS:NTFS /Q" > 																%tmp_file% && type %tmp_file% && type %tmp_file% >> %log_file%
...
echo --------------------------------------------------------------------------------------------------------------- >																	%tmp_file% && type %tmp_file% && type %tmp_file% >> %log_file%
echo. > 																																												%tmp_file% && type %tmp_file% && type %tmp_file% >> %log_file%
title (Step 5 of 6) Performing quick format of drive %drive_letter%:
echo. > 																																												%tmp_file% && type %tmp_file% && type %tmp_file% >> %log_file%
...
echo. > 																																												%tmp_file% && type %tmp_file% && type %tmp_file% >> %log_file%
echo --------------------------------------------------------------------------------------------------------------- >																	%tmp_file% && type %tmp_file% && type %tmp_file% >> %log_file%
echo %date% %time% (Step 6 of 6) Performing Extended SMART test on Drive "%drive_letter%:" >																											%tmp_file% && type %tmp_file% && type %tmp_file% >> %log_file%
echo --------------------------------------------------------------------------------------------------------------- >																	%tmp_file% && type %tmp_file% && type %tmp_file% >> %log_file%
echo. >																																													%tmp_file% && type %tmp_file% && type %tmp_file% >> %log_file%
title (Step 6 of 6) Performing Extended SMART test on Drive "%drive_letter%:"
...
for /f "tokens=*" %%O in ('smartctl -x %drive_letter%:\ ^| find /i "% of test"') do (
		...
		if errorlevel 0 (
		  echo %date% %time% Extended SMART test on Disk "%drive_letter%:" -- %%O ... waiting 30 seconds to poll status again  >														%tmp_file% && type %tmp_file% && type %tmp_file% >> %log_file%
		  goto :extended_smart_in_progress
		) 
	) 
	
echo %date% %time% Extended SMART test complete, proceeding with rest of script  >																										%tmp_file% && type %tmp_file% && type %tmp_file% >> %log_file%
